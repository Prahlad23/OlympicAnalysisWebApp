...
if user_menu == 'Overall Analysis':
    ...
    nations_over_years = helper.data_over_time(df,'region')
    fig = px.line(nations_over_years, x="Edition", y="region")
    st.title("Participating Nations over Time")
    st.plotly_chart(fig)

    events_over_years = helper.data_over_time(df, 'Event')
    events_over_years.rename(columns={'Event': 'Events'}, inplace=True)
    fig = px.line(events_over_years, x="Edition", y="Events")
    st.title("Events over Time")
    st.plotly_chart(fig)

    athletes_over_years = helper.data_over_time(df, 'Name')
    athletes_over_years.rename(columns={'Name': 'Athletes'}, inplace=True)
    fig = px.line(athletes_over_years, x="Edition", y="Athletes")
...
if user_menu == 'Country-wise Analysis':
    ...
    st.title("Medal Tally over the Years for " + str(selected_country))
    st.plotly_chart(fig)

    st.title(selected_country + " in various sports")
...
if user_menu == 'Athlete wise Analysis':
    ...
    fig = ff.create_distplot([x1, x2, x3, x4],
                             ['Overall Age', 'Gold Medalists', 'Silver Medalists', 'Bronze Medalists'], show_hist=False,
                             show_rug=False)

    fig.update_layout(autosize=False,width=1000, height=600)
    st.title("Distribution of Age among Medalists")
    st.plotly_chart(fig)
    ...
    fig.update_layout(autosize=False, width=1000, height=600)
    st.title("Distribution of Age showing Gold Medalists in different Sports")
    st.plotly_chart(fig)
